...
public class KochavaTrackerPlugin extends CordovaPlugin implements AttributionUpdateListener, ConsentStatusChangeListener {
     ...
     */
    @Override
    public final void onAttributionUpdated(final String attribution) {
    ...
    */
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("sendEventString") && args.length() == 2) {
            return sendEventString(optString(args, 0), optString(args, 1));
        }

        //Send an event with map object.
        if (action.equals("sendEventMapObject") && args.length() == 2) {
            return sendEventMapObject(optString(args, 0), optJsonObject(args.opt(1)));
        }
        ...
        if (action.equals("sendEventGooglePlayReceipt") && args.length() == 4) {
            return sendEventGooglePlayReceipt(optString(args, 0), optJsonObject(args.opt(1)), optString(args, 2), optString(args, 3));
        }
        ...
        if (action.equals("sendDeepLink") && args.length() == 2) {
            return sendDeepLink(optString(args, 0), optString(args, 1));
        }
        ...
        if(action.equals("addPushToken") && args.length() == 1) {
            return addPushToken(optString(args, 0));
        }

        //Removes a push token.
        if(action.equals("removePushToken") && args.length() == 1) {
            return removePushToken(optString(args, 0));
        }

        //Sets the sleep state
        if(action.equals("setSleep") && args.length() == 1) {
            return setSleep(optBoolean(args.opt(0)));
        }

        //Retrieves the sleep state.
        if(action.equals("getSleep")) {
            if(callbackContext != null) {
        ...
        //Sets consent required.
        if(action.equals("setConsentRequired") && args.length() == 1) {
            return setConsentRequired(optBoolean(args.opt(0)));
        }
    ...
    private boolean configure(final JSONObject input) {
        ...
        //Check for the partner name old key.
        final String partnerNameOldKey = input.optString("partner_name", null);
        if (partnerNameOldKey != null) {
            configuration.setPartnerName(partnerNameOldKey);
        }
        input.remove("partner_name");

        //Check for the partner name new key.
        final String partnerNameNewKey = input.optString("partnerName", null);
        if (partnerNameNewKey != null) {
            configuration.setPartnerName(partnerNameNewKey);
        }
        input.remove("partnerName");
        ...
        //Check for sleep.
        final Boolean sleep = optBoolean(input.opt("sleepBool"));
        if(sleep != null) {
            configuration.setSleep(sleep);
        }
        input.remove("sleepBool");
        ...
        //Check for the manual consent requirements flag.
        final Boolean consentManualManagedRequirements = optBoolean(input.opt("consentManualManagedRequirements"));
        if(consentManualManagedRequirements != null) {
            configuration.setManualManagedConsentRequirements(consentManualManagedRequirements);
        }
        input.remove("consentManualManagedRequirements");
        ...
        final String urlInitial = input.optString("installNetTransactionURLString", null);
        input.remove("installNetTransactionURLString");
        if(urlInitial != null) {
        ...
        final String urlEvent = input.optString("batchNetTransactionURLString", null);
        input.remove("batchNetTransactionURLString");
        if(urlEvent != null) {
    ...
    private boolean sendEventString(final String eventName, final String eventData) {
        // Note: a null eventName will be dropped but null eventData is fine.
        if(eventData == null) {
    ...
    private boolean sendEventGooglePlayReceipt(final String eventName, final JSONObject eventData, final String receiptJson, final String receiptSignature) {
        // Note: a null eventName will be dropped but null for other parameters is fine.
     ...
     * @return True.
     */
    private boolean sendDeepLink(final String uri, final String sourceApplication) {
        final Event event = new Event(Tracker.EVENT_TYPE_DEEP_LINK);
        event.setUri(uri);
        event.setSource(sourceApplication);
        Tracker.sendEvent(event);
        return true;
    }
     ...
     * @return The sdk version or an empty string if the tracker is not configured.
     */
    private boolean setSleep(Boolean sleep) {
        if(sleep != null) {
    ...
    /**
     * @return The tracker sleep state.
     */
    ...
    */
    ...
    /**
     * Sets consent to required or not as determined by the host app.
     */
    private boolean setConsentRequired(Boolean required) {
        if(required == null) {
            return false;
        }
        Tracker.setConsentRequired(required);
        return true;
    }
    ...
    */
    private String getConsentStatus() {
        ...
        try {
            ...
            consentStatus.put("requirements_known", Tracker.isConsentRequirementsKnown());
        } catch(Throwable t) {
            t.printStackTrace();
        }
